...
import { convertToBuiltInAIMessages } from "./convert-messages";

export class BuddhiAILanguageModel implements LanguageModelV2 {
  readonly specificationVersion = "v2";
  readonly provider = "buddhi-ai";
  ...
  private async getSession(
    ...
    if (availability === ("unavailable" as Availability)) {
    ...
    if (systemMessage) {
      mergedOptions.initialPrompts = [
        { role: "system" as LanguageModelMessageRole, content: systemMessage },
      ];
    }
  ...
  private getArgs({
    ...
    const hasMultiModalInput = this.hasMultimodalContent(prompt);
   ...
   */
  private hasMultimodalContent(prompt: LanguageModelV2Prompt): boolean {
